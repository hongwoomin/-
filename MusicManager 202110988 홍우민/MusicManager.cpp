...
void MusicManager::save_file(string filename) {
    auto it = find_playlist(string("All"));
    ofstream file(filename);
    auto songs = (*it)->get_tracks();
    for (auto k = songs.begin(); k != songs.end(); k++) {
        file << (*k)->get_title() << "," << (*k)->get_title()
        << "," << (*k)->get_album() << "," << (*k)->get_mv_url() << ",";
        auto pl = (*k)->get_playlists();
